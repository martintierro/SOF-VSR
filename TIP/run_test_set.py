...
def parse_args():
    ...
    parser.add_argument('--testset_dir', type=str, default='data/test/Set')
    ...
    with torch.no_grad():
        ...
        for idx_video in range(len(video_list)):
            ...
            for idx_iter, (LR_y_cube, SR_cb, SR_cr) in enumerate(test_loader):
                ...
                if not os.path.exists('results/Set'):
                    os.mkdir('results/Set')
                if not os.path.exists('results/Set/' + cfg.degradation + '_x' + str(cfg.scale)):
                    os.mkdir('results/Set/' + cfg.degradation + '_x' + str(cfg.scale))
                if not os.path.exists('results/Set/' + cfg.degradation + '_x' + str(cfg.scale) + '/' + video_name):
                    os.mkdir('results/Set/' + cfg.degradation + '_x' + str(cfg.scale) + '/' + video_name)
                SR_rgb.save('results/Set/' + cfg.degradation + '_x' + str(cfg.scale) + '/' + video_name + '/sr_' + str(idx_iter+2).rjust(2,'0') + '.png')
